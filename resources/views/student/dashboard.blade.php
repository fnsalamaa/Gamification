...
<div class="container mx-auto mt-8 px-4 max-w-screen-xl">
        ...
        <div class="flex-1">
            ...
            <div class="mt-4">
                <div class="bg-purple-100 text-purple-800 inline-block px-4 py-2 rounded-full text-sm font-semibold">
                    Total Skor: {{ $student->total_score ?? 0 }}
                </div>
            </div>
        </div>
    ...
    <div class="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-gradient-to-r from-yellow-400 to-yellow-500 text-white p-6 rounded-2xl shadow-lg hover:shadow-xl transform hover:scale-105 transition duration-300">
            <div class="flex items-center gap-4 mb-4">
                <a href="{{ route('student.badge.choose') }}" class="flex items-center gap-4">
            ...
            <div class="flex flex-wrap gap-3 mt-2">
                ...
                <button type="button"
                    onclick="showBadgeDetail('{{ $badge->name }}', '{{ $badge->description }}')"
        ...
        {{-- Leaderboard --}}
        <div class="text-white p-6 rounded-2xl shadow-lg hover:shadow-xl transform hover:scale-105 transition duration-300"
            style="background: linear-gradient(to right, #A4DD00, #90C900);">
            <a href="{{ route('student.leaderboard.global') }}">
                <div class="flex items-center gap-4 mb-4">
                    <div class="bg-white text-[#90C900] p-3 rounded-full shadow-md">
                    ...
                    <div>
                        <h4 class="text-2xl font-bold">Top Global Players</h4>
                        <p class="text-sm text-white/90">Ready to make your mark?</p>
                    </div>
                </div>
                ...
                <div class="flex flex-row justify-center items-end gap-4 sm:gap-6 mb-6 overflow-x-auto">
                    ...
                    @if ($student)
                    @php
                        $avatarPath = $student->selectedAvatarModel?->first()?->image_path ?? 'avatars/special.jpg';
                    @endphp
                    <div class="flex flex-col items-center w-20 sm:w-24">
                        <img src="{{ asset('storage/' . $avatarPath) }}"
                            class="w-12 h-12 sm:w-16 sm:h-16 rounded-full border-4 border-white-800 shadow-md">
                        <div class="mt-2 text-xs sm:text-sm font-semibold text-white text-center truncate max-w-[5rem] sm:max-w-[6rem]">
                            {{ $student->user->name }}
                        </div>
                        <div class="text-[10px] sm:text-xs text-white/90 mb-2">Score: {{ $student->total_score }}</div>
